...
async def test_discover_azure_only(monkeypatch):
    ...
    with (
        patch("src.tools.tools.settings") as mock_settings,
        patch(
            "src.providers.azure.list_containers", new=AsyncMock(return_value=[{"name": "sales"}])
        ),
        ...
        containers = await __import__(
            "src.providers.azure", fromlist=["list_containers"]
        ).list_containers()
...
async def test_discover_returns_provider_errors_gracefully():
    ...
    with patch(
        "src.providers.s3.list_buckets", new=AsyncMock(side_effect=RuntimeError("no creds"))
    ):
